#command to #put text from many files into a #file
```bash
cat "app/(store)/layout.tsx" "components/ClientLayout.tsx" "components/ui/Navbar.tsx" "middleware.ts" > "app/merged_file.txt"
```



#zsh #scann #through #files to #find a #word in different #case 
```zsh
grep -EriIl "clerk|Clerk" app components context lib sanity middleware.ts --exclude-dir=node_modules
```
...
#show #corrupted #files #with #errors ─ tsc --noEmit
```bash
tsc --noEmit  
```

#bot #systemd #management 

Here’s how to configure `systemd` for your bot running under the `root` user with the specified path.
...
2. Add the following configuration:
    ...
    ```ini
    [Unit]
    Description=Telegram One-to-Many Bot
    After=network.target
    
    [Service]
    ExecStart=/usr/bin/python3 /root/bot/telegram_one_to_many_bot/bots/bot.py
    Restart=always
    RestartSec=5
    WorkingDirectory=/root/bot/telegram_one_to_many_bot/bots
    User=root
    ...
    **Explanation:**
    ...
    - `ExecStart`: Command to start your bot.
    - `Restart`: Ensures the bot restarts automatically after a crash.
    - `RestartSec`: Wait time before restarting.
    - `WorkingDirectory`: Path where the bot script resides.
    - `User`: Runs the service as the `root` user.
    - `Environment`: Ensures logs are printed immediately without buffering.
...
3. Enable the bot service to start on boot:
...
You should see the bot running. If there are any errors, they will be displayed here.

---

### Step 4: View Logs

To monitor the bot logs, use:
...
### Step 5: Stop or Restart the Service (if needed)
...
Your bot is now set up to run as a `systemd` service. It will restart automatically if it crashes and start automatically after a reboot.
