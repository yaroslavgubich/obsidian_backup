...
```
cd ~/code/yaroslavgubich/fullstack-challenges/04-Front-End/01-HTML-and-CSS/04-Responsive-profile
cp -r profile ~/code/yaroslavgubich
cd ~/code/yaroslavgubich/profile
```
...
Now you can visit the URL `http://yaroslavgubich.github.io/profile/` (this is the URL built automatically by GitHub) and have a look at your masterpiece online! Share the link on Slack with your buddies.

From now and until the end of the day, you can keep working in your `~/code/yaroslavgubich/profile` directory AND on the `gh-pages` branch. This means any updates of your profile can be pushed on `http://yaroslavgubich.github.io/profile/` through usual git commands:
...
```
cd ~/code/yaroslavgubich/fullstack-challenges/04-Front-End/01-HTML-and-CSS/05-Push-on-Github-Pages
cp -r ~/code/yaroslavgubich/profile .
rm -rf profile/.git
git add .
